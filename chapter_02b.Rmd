...
---
title: Introduction to <grn>mrgsolve</grn> part 1
author: Kyle T. Baron
...
tryit_file <- "chapter_02_workbook.Rmd"
```

```{r setup, echo = FALSE, message = FALSE}
mod <- mread_cache("popex", modlib()) %>% 
  update(end = 192, delta = 0.2) %>% Req(CP = DV)
data(exidata)
...
It's called a <grn>pipe</grn>.  It allows us to pass the output from one function 
into another function ... creating a pipeline
...
```{r}
mod %>% ev(amt = 100) %>% mrgsim() %>% plot(CP~., col = "firebrick")
```
...
# Simulation end time
```{r}
mod %>% ev(amt = 100) %>% mrgsim(end = 96) %>% plot(CP~time)
```

# Simulation time step
```{r}
mod %>% ev(amt = 100) %>% mrgsim(end = 96, delta = 0.1) %>% plot(CP~time)
```
...
`mrgsim` will call `update` for you
```{r, eval = FALSE}
mod %>% mrgsim(end = 72)
```
