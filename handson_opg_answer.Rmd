...
## tells you what is missing that is expected
inventory(mod,post)
```
...
```{r}
## put in different matrices for each simulation
omegas <- as_bmat(post,"OMEGA")
sigmas <- as_bmat(post,"SIGMA")
## look at an example of the 1000 matrices
## omegas[[1]]
slice(post, 10)
```
...
```{r}
## tgrid creates a time grid for you
# stime(tgrid(0,12,6) + 24)

## end=-1 used to only use addl draw times
...
sim <- function(i,data,des) {
  ## grabs model file from global environment BAD CODING PRACTICE TO DISREGARD SCOPING
  mod %<>% 
    param(slice(post,i)) %>%
    ## update RE matrices
    omat(omegas[[i]]) %>% 
...
mcRNG()
set.seed(22223)
## can't use multiple cores on Windows :(
...
```
## this tells us that precise estimates of Q are not necessary to get an accurate estimate of median change from baseline; however, TVVMAX and TVIC50 will definitely impact the estimate of median change 
