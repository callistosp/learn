...
e1 <- ev(amt = 500, ii = 24)
e2 <- ev(amt = 250, ii = 24, addl = 3)
dose <- seq(e1,e2)

mod %>% mrgsim(events = dose, end = 24*21) %>% plot()
```
...
```{r}
## zero_re() zeroes out the random effects in the model
...
blocks(mod)
init(mod)
see(mod)     ## [THETA] tells us that time unit is days

## look at change in PASI w/ and w/o dosing
mrgsim(mod)
mrgsim(mod, events = ev(amt=100))


## look at initial model attempt
out <- mod %>% ev(amt = 25) %>% mrgsim(end = 35) 
plot(out)
tail(out)

## look at diff events as req by clin pharm
data <- expand.ev(amt = seq(0,150,5), ii = 7, addl = 4)
out <- 
  mod %>%
  data_set(data) %>%
  mrgsim(end = 35) %>%
...
# ## two times, so need to use workaround, or mutate...
# sims <- left_join(out, data, by="ID", 
#                   suffix = c("", ".y"))
...
## can also get rid of filter call
## end=-1 ignores time grid, only simulates time 35 this way, 
## instead of all times up to 35 also
out <- 
  mod %>%
  data_set(data) %>%
  carry_out(dose, amt) %>%
...
ggplot(data = out, aes(dose, dPASI)) +
  geom_line()
...
writeLines(mrgsolve:::code(mod), "mypopex.cpp")
```
=======
## Sim EBE
