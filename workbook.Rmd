...
You are the pharmacometrician supporting secukinumab development for psoriasis, 
...
It has log-normal IIV and combined additive / portional error model.  You got the
covariance matrix and the condition number was low.  The pred-corrected VPC 
...
faces of your team members. 

The clin pharm lead clears her throat: Um, we'd like to push the dose as high 
as we can to get to PASI75 by week 5.  Can you tell us what dose that might
be?
...
```{r}
mod <- mread_cache("secuk", "model") %>% zero_re()

see(mod)

out <- mod %>% ev(amt = 75, ii = 7, addl = 4) %>% mrgsim() 
